...
class VisaQuoteRequestModel(models.Model):
    ...
    uuid = models.UUIDField(db_index=True, default=uuid.uuid4, editable=False, unique=True)
    expat = models.ForeignKey(ExpatModel, on_delete=models.SET_NULL, blank=True, null=True, related_name='requests')
    date = models.DateTimeField(auto_now_add=True)
    persons = models.IntegerField(default=1)
    service = models.ForeignKey(Service, on_delete=models.DO_NOTHING)
    note = models.TextField()
    visa = models.ForeignKey(VisaModel, on_delete=models.DO_NOTHING)
...
class VisaQuoteResponseModel(models.Model):
    agent = models.ForeignKey(VisaAgencyModel, on_delete=models.DO_NOTHING)
